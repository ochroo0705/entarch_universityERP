...
export default function TeacherAnnouncements() {
  ...
  useEffect(() => {
    getAnnouncements()
      .then((res) => {
        // Filter to show announcements targeting teachers (roleFlags & 2)
        const all = res.data || [];
        const relevant = all.filter(
          (a) => !a.targetRoleFlags || (a.targetRoleFlags & 2) !== 0
        );
  ...
  if (loading) return <div className="loading"><div className="spinner" />{t('teacher.announcements.loading')}</div>;

  return (
    <div>
      <div className="page-header">
        <h1>{t('teacher.announcements.title')}</h1>
      </div>
