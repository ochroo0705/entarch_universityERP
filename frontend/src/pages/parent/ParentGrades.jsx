...
import { ParentGradesSkeleton } from '../../components/ui/ParentPageSkeletons';
...
export default function ParentGrades() {
  ...
  const quarterLabels = {
    1: t('parent.grades.q1'),
    2: t('parent.grades.q2'),
    3: t('parent.grades.q3'),
    4: t('parent.grades.q4'),
  };
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      getStudentGrades(selectedChild.id).catch(() => ({ data: [] })),
      getGradeTrends(selectedChild.id).catch(() => ({ data: null })),
    ])
  ...
  const grades = useMemo(
    () => (selectedQuarter ? allGrades.filter((grade) => grade.quarter === selectedQuarter) : allGrades),
    [allGrades, selectedQuarter]
  );
  ...
  const { avgGrade, highestGrade, lowestGrade } = useMemo(() => {
    const values = grades.filter((grade) => grade.gradeValue != null).map((grade) => grade.gradeValue);
    return {
      avgGrade: values.length > 0 ? (values.reduce((sum, value) => sum + value, 0) / values.length).toFixed(1) : null,
      highestGrade: values.length > 0 ? Math.max(...values) : null,
      lowestGrade: values.length > 0 ? Math.min(...values) : null,
    };
  }, [grades]);

  if (childLoading) return <div className="loading"><div className="spinner" />{t('common.loading')}</div>;
  if (!selectedChild) return <div className="card"><div className="card-body" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '3rem' }}>{t('parent.grades.noChildren')}</div></div>;

  const childName = `${selectedChild.firstName} ${selectedChild.lastName}`;

  return (
    <div>
      <div className="page-header"><h1>{t('parent.grades.title', { name: childName })}</h1></div>
      <ChildSelector />

      <div className="content-stack" aria-busy={loading}>
        {loading ? (
          <ParentGradesSkeleton />
        ) : (
          <>
            <div className="parent-hero-grid">
              <div className="parent-stack">
                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                  {selectedQuarter ? t('parent.grades.quarterStats', { n: selectedQuarter }) : t('parent.grades.allQuarterStats')}
                </span>
                <div className="parent-mini-stats-grid">
                  <div className="stat-card">
                    <div className="stat-icon teachers">{'\u{1F4CA}'}</div>
                    <div className="stat-info">
                      <h3 style={{ color: avgGrade ? getGradeColor(parseFloat(avgGrade)) : undefined }}>{avgGrade || '-'}</h3>
                      <p>{t('parent.grades.average')}</p>
                    </div>
                  </div>
                  <div className="stat-card">
                    <div className="stat-icon students">{'\u{1F3C6}'}</div>
                    <div className="stat-info">
                      <h3 style={{ color: highestGrade ? getGradeColor(highestGrade) : undefined }}>{highestGrade ?? '-'}</h3>
                      <p>{t('parent.grades.highest')}</p>
                    </div>
                  </div>
                  <div className="stat-card">
                    <div className="stat-icon assignments">{'\u{1F4C9}'}</div>
                    <div className="stat-info">
                      <h3 style={{ color: lowestGrade ? getGradeColor(lowestGrade) : undefined }}>{lowestGrade ?? '-'}</h3>
                      <p>{t('parent.grades.lowest')}</p>
                    </div>
                  </div>
                  <div className="stat-card">
                  ...
                  <div className="card parent-chart-card" style={{ marginBottom: 0 }}>
                    <div className="card-body" style={{ padding: '0.75rem 1rem' }}>
                      <h3 style={{ fontSize: '0.9rem', marginBottom: '0.4rem', fontFamily: 'var(--font-heading)', fontWeight: 700 }}>
                        {t('parent.grades.performanceTrend')}
                      </h3>
                      <div className="parent-chart-frame">
                        <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 'auto', overflow: 'visible' }}>
                          ...
                          <path d={areaStr} fill="url(#parentAreaGrad)">
                          ...
                          {points.map((point, index) => (
                            <g key={index}>
                              ...
                              <text x={point.x} y={padT + chartH + 18} textAnchor="middle" fontSize="11" fontWeight="600" fill="var(--text-muted)" fontFamily="var(--font-heading)">
                                {quarterLabels[point.quarter] || `Q${point.quarter}`}
                              </text>
                            </g>
                          ))}
                        </svg>
            ...
            <div className="card" style={{ marginBottom: '1rem' }}>
              <div className="card-body parent-filter-row" style={{ padding: '0.75rem 1rem' }}>
                <span style={{ fontWeight: 600, fontSize: '0.85rem', marginRight: '0.5rem' }}>{t('parent.grades.quarterLabel')}</span>
                <button className={`btn btn-sm ${selectedQuarter === null ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setSelectedQuarter(null)}>{t('parent.grades.all')}</button>
                {[1, 2, 3, 4].map((quarter) => (
                  <button key={quarter} className={`btn btn-sm ${selectedQuarter === quarter ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setSelectedQuarter(quarter)}>
            ...
            {Object.keys(bySubject).length === 0 ? (
              <div className="card">
                <div className="card-body" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '3rem' }}>
                  {t('parent.grades.noGrades')}
                </div>
              </div>
            ) : (
              <div className="parent-stack">
                {Object.entries(bySubject).map(([subject, subjectGrades], subjectIndex) => {
                  const subjectValues = subjectGrades.filter((grade) => grade.gradeValue).map((grade) => grade.gradeValue);
                  const avg = subjectValues.length > 0 ? (subjectValues.reduce((sum, value) => sum + value, 0) / subjectValues.length).toFixed(0) : null;
                    ...
                    <div key={subject} className="card" style={{ animation: `fadeInUp 0.35s cubic-bezier(0.16,1,0.3,1) ${subjectIndex * 0.05}s both` }}>
                      <div className="card-body">
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem', gap: '0.75rem', flexWrap: 'wrap' }}>
                          ...
                          {avg ? <span style={{ fontWeight: 700, fontSize: '1.1rem', color: getGradeColor(parseInt(avg, 10)) }}>{avg}</span> : null}
                        </div>
                        <div className="parent-subject-grade-grid">
                          {[...subjectGrades].sort((a, b) => (a.quarter || 0) - (b.quarter || 0)).map((grade) => (
                            <div key={grade.id} style={{ padding: '0.6rem 0.75rem', borderRadius: '6px', border: '1px solid var(--border)', background: 'rgba(246,244,240,0.5)' }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem', gap: '0.5rem', flexWrap: 'wrap' }}>
                                <span style={{ fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase', color: GRADE_COLORS[grade.gradeType] || 'var(--text-muted)' }}>
                                  {grade.gradeType}
                                </span>
                                <span className="badge badge-info" style={{ fontSize: '0.65rem' }}>
                                  {quarterLabels[grade.quarter] || `Q${grade.quarter}`}
                                </span>
                              </div>
